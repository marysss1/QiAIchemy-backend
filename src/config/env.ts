...
const envSchema = z.object({
  ...
  RAG_GRAPH_MIN_CONFIDENCE: z.coerce.number().min(0).max(1).default(0.24),
  RAG_GRAPH_MAX_WEIGHT: z.coerce.number().min(0).max(0.4).default(0.16),
  RAG_GRAPH_RRF_WEIGHT: z.coerce.number().min(0).max(1.2).default(0.4),
